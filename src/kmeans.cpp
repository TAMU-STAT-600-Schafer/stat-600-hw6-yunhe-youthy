...
arma::uvec MyKmeans_c(const arma::mat& X, int K,
    ...
    arma::rowvec k_ones = arma::ones<arma::rowvec>(K);
    ...
    for (int i = 0; i < numIter; i++){
      /* Iteration Step I: M-Step (Assignment)
       Preparations */
      arma::rowvec M_norms = arma::sum(pow(M_iter, 2), 0);
      arma::mat XM = X * M_iter; // ij-th element: x_i^Tc_j
      arma::mat dists = X_norms * k_ones + n_ones * M_norms - 2 * XM; // matrix between samples and centroids
      //Compute and assign vectors to every current cluster
      Y = arma::index_min(dists, 1) + 1;

      // Iteration Step II: E-Step (New Centers)
      arma::mat M_temp(p, K, arma::fill::zeros); // new centroids storage (temporary)
      for (int j = 0; j < K; j++){
        arma::uvec index = arma::find(Y == j + 1);
        if (index.n_elem == 0) {
          throw std::runtime_error("ERROR: Bad Initial Centroids!");
        } else{
          M_temp.col(j) = arma::mean(X.rows(index), 0).t();
        }
      }
